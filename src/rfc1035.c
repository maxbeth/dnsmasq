	      ...
	      if (aqclass == C_IN && res != 2 && (aqtype == T_CNAME || aqtype == qtype))
		{
		  ...
		  if (aqtype == T_CNAME)
		      ...
		      if (ipsets && (flags & (F_IPV4 | F_IPV6)))
			{
		      ...
		      if (flags & (F_IPV6 | F_IPV4) && option_bool(OPT_LOG_INS_CACHE)) {
		        char *ipadr = safe_malloc(ADDRSTRLEN);
		        char *ipname = safe_malloc(MAXDNAME);
		        if (flags & F_IPV4) {
			    inet_ntop(AF_INET, &(addr.addr.addr4), ipadr, ADDRSTRLEN);
		        } else {
			    inet_ntop(AF_INET6, &(addr.addr.addr6), ipadr, ADDRSTRLEN);
		        }
		        extract_name(header, qlen, &p, ipname, 1, 0);
			my_syslog(LOG_INFO, "CACHE: Name:%s TTL:%lu IP:%s", ipname, attl, ipadr);
		        free(ipname);
		        free(ipadr);
		      }
		      if (newc && cpp)
